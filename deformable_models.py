...
#long = [50, 240]
#top = [65, 180]
centers = np.array([[215, 170], [225, 90], [260, 130], [115, 175], [260, 285]])
radius1 = 0.07*np.mean(im.shape)
B = sis.regularization_matrix(nr_points, alpha, beta)
B1 = sis.regularization_matrix(nr_points, alpha1, beta2)
...
snakes_5= sis.make_circular_snake(nr_points, centers[4], radius)
...
ax.plot(np.r_[snakes_5[1],snakes_5[1,0]],np.r_[snakes_5[0],snakes_5[0,0]],'c-')
ax.set_title('Initialization')
...
for j in range(nr_iter):
    ...
    snakes_4 = sis.evolve_snake2(snakes_4, im_full[0,:,:], B1, step_size)
    snakes_5 = sis.evolve_snake(snakes_5, im_full[0,:,:], B, step_size) 

snakes3D_1.append(snakes_1)
snakes3D_2.append(snakes_2)  
snakes3D_3.append(snakes_3)  
snakes3D_4.append(snakes_4)  
snakes3D_5.append(snakes_5)  
...
ax.plot(np.r_[snakes3D_5[0][1],snakes3D_5[0][1,0]],np.r_[snakes3D_5[0][0],snakes3D_5[0][0,0]],'c-')
...
for i in range(2,900):
    for j in range(nr_iter2):
        ...
        snakes_4 = sis.evolve_snake2(snakes_4, im_full[i,:,:], B1, step_size)
        snakes_5 = sis.evolve_snake(snakes_5, im_full[i,:,:], B, step_size)
...
for j in imgs:
    ...
    ax.plot(np.r_[snakes3D_5[j][1],snakes3D_5[j][1,0]],np.r_[snakes3D_5[j][0],snakes3D_5[j][0,0]],'y-')
    ax.axis('off')

#%%
for j in [0, 290, 295, 298]:
    fig, ax = plt.subplots()
    fig.tight_layout()
    ax.imshow(im_full[j,:,:], cmap=plt.cm.gray)
    ax.plot(np.r_[snakes3D_1[j][1],snakes3D_1[j][1,0]],np.r_[snakes3D_1[j][0],snakes3D_1[j][0,0]],'r-', linewidth = 2)
    ax.plot(np.r_[snakes3D_2[j][1],snakes3D_2[j][1,0]],np.r_[snakes3D_2[j][0],snakes3D_2[j][0,0]],'g-', linewidth = 2)
    ax.plot(np.r_[snakes3D_3[j][1],snakes3D_3[j][1,0]],np.r_[snakes3D_3[j][0],snakes3D_3[j][0,0]],'b-', linewidth = 2)
    ax.plot(np.r_[snakes3D_5[j][1],snakes3D_5[j][1,0]],np.r_[snakes3D_5[j][0],snakes3D_5[j][0,0]],'y-')
    ax.axis('off')
...
ax.plot(np.r_[snakes3D_2[j][1],snakes3D_2[j][1,0]],np.r_[snakes3D_2[j][0],snakes3D_2[j][0,0]],'g-', linewidth = 2)
ax.plot(np.r_[snakes3D_3[j][1],snakes3D_3[j][1,0]],np.r_[snakes3D_3[j][0],snakes3D_3[j][0,0]],'b-', linewidth = 2)
ax.plot(np.r_[snakes3D_4[j][1],snakes3D_4[j][1,0]],np.r_[snakes3D_4[j][0],snakes3D_4[j][0,0]],'y-', linewidth = 2)
ax.plot(np.r_[snakes3D_5[j][1],snakes3D_5[j][1,0]],np.r_[snakes3D_5[j][0],snakes3D_5[j][0,0]],'y-')
...
X1 = np.r_[snakes3D_1[j][1],snakes3D_1[j][1,0]]
Y1 = np.r_[snakes3D_1[j][0],snakes3D_1[j][0,0]]
X2 = np.r_[snakes3D_2[j][1],snakes3D_2[j][1,0]]
Y2 = np.r_[snakes3D_2[j][0],snakes3D_2[j][0,0]]
X3 = np.r_[snakes3D_3[j][1],snakes3D_3[j][1,0]]
Y3 = np.r_[snakes3D_3[j][0],snakes3D_3[j][0,0]]
X4 = np.r_[snakes3D_4[j][1],snakes3D_4[j][1,0]]
Y4 = np.r_[snakes3D_4[j][0],snakes3D_4[j][0,0]]
X5 = np.r_[snakes3D_5[j][1],snakes3D_5[j][1,0]]
Y5 = np.r_[snakes3D_5[j][0],snakes3D_5[j][0,0]]
n1 = len(X1)
n2 = len(X2)
n3 = len(X3)
n4 = len(X4)
n5 = len(X5)
A1 = polygonArea(X1, Y1, n1)
A2 = polygonArea(X2, Y2, n2)
A3 = polygonArea(X3, Y3, n3)
A4 = polygonArea(X4, Y4, n4)
A5 = polygonArea(X5, Y5, n5)
S = (A1+A2+A3+A4+A5)/5
R = np.sqrt(S/np.pi)
print(S)
print(R)
